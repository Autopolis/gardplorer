  ...
  >
    <el-table-column label="TX HASH">
      <template slot-scope="scope">
        <hg-link type="tx" :content="scope.row.hash" />
      </template>
    </el-table-column>

    <el-table-column label="BLOCK">
      <template slot-scope="scope">
        <hg-link type="block" :content="scope.row.height" />
      </template>
    </el-table-column>

    <el-table-column label="FROM">
      <template slot-scope="scope">
        <hg-link type="address" :content="scope.row.input.address" />
      </template>
    </el-table-column>

    <el-table-column label="TO">
      <template slot-scope="scope">
        <hg-link type="address" :content="scope.row.output.address" />
      </template>
    </el-table-column>
  ...
  watch: {
    list: function () {
      if (!isEmpty(this.list)) {
        this.list.forEach(item => { this.$store.dispatch('blocks/fetchDetail', get(item, 'height')) });
      }
    }
  }
}
